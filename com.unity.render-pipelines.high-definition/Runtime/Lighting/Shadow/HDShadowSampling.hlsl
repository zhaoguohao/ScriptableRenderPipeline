...
real SampleShadow_PCF_Tent_3x3(float4 shadowAtlasSize, float3 coord, real2 sampleBias, Texture2D tex, SamplerComparisonState compSamp)
{
...
real SampleShadow_PCF_Tent_5x5(float4 shadowAtlasSize, float3 coord, real2 sampleBias, Texture2D tex, SamplerComparisonState compSamp)
{
#if SHADOW_USE_DEPTH_BIAS == 1
    // add the depth bias
    coord.z += depthBias;
...
//
real SampleShadow_PCF_Tent_7x7(float4 shadowAtlasSize, float3 coord, real2 sampleBias, Texture2D tex, SamplerComparisonState compSamp)
{
#if SHADOW_USE_DEPTH_BIAS == 1
    // add the depth bias
    coord.z += depthBias;
