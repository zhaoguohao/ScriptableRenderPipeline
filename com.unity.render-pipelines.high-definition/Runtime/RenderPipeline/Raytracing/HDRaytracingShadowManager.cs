...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    ...
    public class HDRaytracingShadowManager
    {
        ...
        public bool RenderAreaShadows(HDCamera hdCamera, CommandBuffer cmd, ScriptableRenderContext renderContext)
        {
            ...
            m_KernelFilter = bilateralFilter.FindKernel("AreaBillateralShadow");
            ...
            for(int lightIdx = 0; lightIdx < numLights; ++lightIdx)
            {
                ...
                using (new ProfilingSample(cmd, "Raytrace Area Shadow", CustomSamplerId.Raytracing.GetSampler()))
                {
                    ...
                    cmd.SetRaytracingTextureParam(shadowsShader, m_RayGenShaderName, HDShaderIDs._GBufferTexture[0], m_GbufferManager.m_RTs[0]);
                    cmd.SetRaytracingTextureParam(shadowsShader, m_RayGenShaderName, HDShaderIDs._GBufferTexture[1], m_GbufferManager.m_RTs[1]);
                    cmd.SetRaytracingTextureParam(shadowsShader, m_RayGenShaderName, HDShaderIDs._GBufferTexture[2], m_GbufferManager.m_RTs[2]);
                    cmd.SetRaytracingTextureParam(shadowsShader, m_RayGenShaderName, HDShaderIDs._GBufferTexture[3], m_GbufferManager.m_RTs[3]);
