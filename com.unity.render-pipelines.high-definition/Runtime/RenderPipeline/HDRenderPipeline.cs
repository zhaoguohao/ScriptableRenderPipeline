    ...
    public class HDRenderPipeline : UnityEngine.Rendering.RenderPipeline
    {
        ...
        void SendGraphicsBuffers(CommandBuffer cmd, HDCamera hdCamera)
        {
            ...
            if (needNormalBuffer)
            {
                ...
                {
                    return rtHandleSystem.Alloc(Vector2.one, colorFormat: mainNormalBuffer.rt.format,
                        sRGB: mainNormalBuffer.rt.sRGB, enableRandomWrite: false, name: $"Normal History Buffer"
                    );
                }
            ...
            if (needDepthBuffer)
            {
                ...
                RTHandleSystem.RTHandle Allocator(string id, int frameIndex, RTHandleSystem rtHandleSystem)
                {
                    return rtHandleSystem.Alloc(Vector2.one, colorFormat: mainDepthBuffer.rt.format,
                        sRGB: mainDepthBuffer.rt.sRGB, enableRandomWrite: false, name: $"Depth History Buffer"
                    );
                }
