...
#define DECLARE_DBUFFER_TEXTURE(NAME)	\
	TEXTURE2D_ARRAY(MERGE_NAME(NAME, 0));		\
	TEXTURE2D_ARRAY(MERGE_NAME(NAME, 1));		\
	TEXTURE2D_ARRAY(MERGE_NAME(NAME, 2));		\
	TEXTURE2D_ARRAY(MERGE_NAME(NAME, 3));

#define FETCH_DBUFFER(NAME, TEX, unCoord2)												\
	DBufferType0 MERGE_NAME(NAME, 0) = LOAD_TEXTURE2D_ARRAY(MERGE_NAME(TEX, 0), unCoord2, unity_StereoEyeIndex);	\
	DBufferType1 MERGE_NAME(NAME, 1) = LOAD_TEXTURE2D_ARRAY(MERGE_NAME(TEX, 1), unCoord2, unity_StereoEyeIndex);	\
	DBufferType2 MERGE_NAME(NAME, 2) = LOAD_TEXTURE2D_ARRAY(MERGE_NAME(TEX, 2), unCoord2, unity_StereoEyeIndex);	\
	DBufferType3 MERGE_NAME(NAME, 3) = LOAD_TEXTURE2D_ARRAY(MERGE_NAME(TEX, 3), unCoord2, unity_StereoEyeIndex).xy; // Caution: We do .xy to match #define DBufferType3 float2 and avoid a shader warning
