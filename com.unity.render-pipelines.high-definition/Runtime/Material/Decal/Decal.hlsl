...
#define DECLARE_DBUFFER_TEXTURE(NAME)	\
	UNITY_DECLARE_SCREENSPACE_TEXTURE(MERGE_NAME(NAME, 0));		\
	UNITY_DECLARE_SCREENSPACE_TEXTURE(MERGE_NAME(NAME, 1));		\
	UNITY_DECLARE_SCREENSPACE_TEXTURE(MERGE_NAME(NAME, 2));		\
	UNITY_DECLARE_SCREENSPACE_TEXTURE(MERGE_NAME(NAME, 3));

#define FETCH_DBUFFER(NAME, TEX, unCoord2)												\
	DBufferType0 MERGE_NAME(NAME, 0) = LOAD_TEXTURE(MERGE_NAME(TEX, 0), unCoord2);	\
	DBufferType1 MERGE_NAME(NAME, 1) = LOAD_TEXTURE(MERGE_NAME(TEX, 1), unCoord2);	\
	DBufferType2 MERGE_NAME(NAME, 2) = LOAD_TEXTURE(MERGE_NAME(TEX, 2), unCoord2);	\
	DBufferType3 MERGE_NAME(NAME, 3) = LOAD_TEXTURE(MERGE_NAME(TEX, 3), unCoord2).xy; // Caution: We do .xy to match #define DBufferType3 float2 and avoid a shader warning
